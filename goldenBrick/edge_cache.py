...
class EC:
    ...
    def __init__(self, csr_filename):
        ...
        colIndexVals = np.genfromtxt(csr_filename, delimiter=',')[0][:-1]
        rowIndexVals = np.genfromtxt(csr_filename, delimiter=',')[1]
    ...
    def one_clock(self):
        if io_port.cc_ec_edgeAddr < 32:
            io_port.ec_rdData_n = self.colIndex[io_port.cc_ec_edgeAddr : io_port.cc_ec_edgeAddr + 8]
        else: 
            io_port.ec_rdData_n = self.rowIndex[(io_port.cc_ec_edgeAddr-32) : (io_port.cc_ec_edgeAddr-32) + 8]